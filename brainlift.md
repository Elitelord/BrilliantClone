...
- **Spiky POV 1:** We shouldn't try to make learning fun. 
  - **Elaboration:** According to the science of learning, higher levels of difficulty along with assessment and other testing forms both contribute to better recall capability. These practices usually don't make for "fun" learning but they do make it quicker. Focusing on the optimal difficulty as the goal instead of focusing on fun learning provides better outcomes in shorter time. From subcategory 1.8, we also know that time on task isnt a great indicator of learning, this is why focusing on fun isn't a priority. 
- **Spiky POV 2:** The application of AI into educational apps and teaching shouldn't operate on a large scale autonomously.
  - **Elaboration:** Guides, verification layers, and human observation are all extremely important as seen by tendencies to hallucinate. Scoping AI into smaller-scoped interactions reduces the risk of hallucinations, acting as a safety layer. Additionally, tailoring is needed for different content areas. Allowing AI to have a chance of hallucinating has been show by research to be incredibly harmful for learner's education, propogating miconceptions. 
- **Spiky POV 3:** Gamification targets the wrong metric in implemenataion. 
  - **Elaboration:** As seen in Category 3's research of other apps, they largely use Gamification methods to increase engagement and consistency. Engagement isn't the right metric to be looking at  as it doesn't correlate to retention. Maximizing for it to increase time-on-app can increase cognitive offloading. The consistency aspect is strong, yet in a future where public institutions integrate educational apps, the profit motive shouldn't drive the app's design. These apps should be solely for educational benefit and consistency will be enforced through external methods. This will improve relative effectiveness of the apps. 
...
- **Insight 1:** A lot of learning comes at a specific middle point, many tactics like spacing and interleaving can be ineffective at extreme ends. These require a balanced approach to have optimal effect. The optimal spacing gap actually scales with how long you need to remember something (Cepeda et al. 2006), and desirable difficulties like these only end up helping if the learner can still overcome them (Bjork & Bjork 2011). 
- **Insight 2:** Oftentimes, difficult assessments and testing are the most effective provided they aren't too difficult that recall fails entirely. Repeated testing beat repeated studying ~61% vs 40% at a one-week delay, even without any feedback (Roediger & Karpicke 2006).
- **Insight 3:** The thing that ties retrieval, spacing, and interleaving together is that performance during practice and actual learning are often inversely related. Interleaved practice looked worse during the practice session but hit 63% vs 20% on the later test (Rohrer & Taylor 2007), and plain re-studying raised learners' confidence while producing worse retention. This means the strategies that feel the most productive are usually the least effective ones (Dunlosky et al. 2013).
- **Insight 4:** Doing dominates almost every other variable. The "doer effect" found that doing an activity predicted learning ~6x more than reading and ~3x more than watching video (Koedinger et al. 2015). This means the choice between interactive and passive content matters more than nearly any feature or content tweak layered on top of it. 

### From Applying AI to Learning

- **Insight 1:** Chatbot based AI learning is the most widely desired, with heavy marketing surrounding it. However, there are many valid questions regarding effectiveness. Specific studies show that it can be "near-human" levels, but recent platform integrations haven't shown as strong results. Looking closer, every successful study (Kestin et al. 2025, Tutor CoPilot 2024, World Bank Nigeria 2025) shared a hidden property: the AI was scaffolded, scoped, and positioned as a tutor that doesn't just hand over answers. "Just use ChatGPT" was never what got tested, so the evidence really only backs the narrow version of the claim, not the broad one the marketing implies. 
- **Insight 2:** With a proper framework, AI has been proven to be effective for feedback and limited scope interactions. Research shows that attempting to generalize it largely increases Hallucination. The biggest proven win is actually formative feedback at scale, which is one of the highest-leverage things in all of learning (Hattie & Timperley 2007, effect size ~0.79), but the place AI fails hardest is scoring and judgment, where zero-shot grading is biased and only matched humans ~30% of the time. The risk lands on a clean line: let AI draft, but let deterministic logic or a human decide.
- **Insight 3:** Adaptive/personalized learning is the most hyped and least independently proven layer of all of this. Knewton raised $182M+ on a "robot tutor in the sky" and got sold for a fraction of it, even a solid system like MATHia took a full year to show a ~0.2 SD gain (RAND 2014), and Squirrel AI's biggest numbers are vendor-reported rather than independent RCTs. Deterministic mastery gating seems to capture most of the benefit without taking on the hallucination risk. 
...
- **Insight 2:** Many apps, including this one, strategize each lesson as data, with duolingo these are more modular as they go question by question. This engineering decision makes it much easier to add features on, adapt lessons for newer styles, and tune lessons to the user's understanding and knowledge. The bigger realization is that this one decision pays off twice: content-as-data is both a pedagogy choice (you can mass-produce and human-review problem variations) and the precondition that makes grounding and verifying any AI even possible. Teams that bolt AI onto unstructured content can't really do either.
- **Insight 3:** The safety measure is the verifier, not the model itself. This project can swap between providers (OpenAI, Gemini, Firebase) without touching any feature code precisely because the truth lives in `dtm.ts` and not in the model. Once the correctness check is deterministic, which specific model writes the question stops mattering much, and the worst case is just a missing question rather than a wrong one taught as right.
...
- **FRQ practice** (`/frq`, `src/content/frqBank.ts`) — AP-style free-response sets (no-/one-/two-stimulus) where *identify* parts are verified deterministically against `dtm.ts` and *describe/explain* parts are rubric-graded by reusing the explain-back grader (`gradeExplanation`), always revealing the official-style rubric + model answer. With AI off it degrades to self-grading against that rubric, so it still teaches — the same additive-AI + verify-what's-checkable discipline applied to exam writing.
