...
- **Steps (10 — explore-led):**
  ...
  10. **`country-model` (UAE two-chart drag)** *(explore — back to the DTM)* — compare the UAE's real boom against the textbook DTM path; migration, not births/deaths, drove it. Read-only (no graded problem), so it's an explore. *(reused asset)*
